...
class HFTEngine:
    ...
    def _update_daily_profit_protection(self):
        ...
        if self.daily_protect_active:
            ...
            # Verifica se PnL caiu abaixo do piso (com histerese: 3 checks consecutivos)
            if pnl <= self.daily_protect_floor and pnl < self.peak_daily_pnl:
                self._daily_protect_below_count = getattr(self, '_daily_protect_below_count', 0) + 1
                if not self.daily_protect_stopped and self._daily_protect_below_count >= 3:
                    ...
                    log.info(f'  🛡 DAILY PROTECT aviso: PnL ${pnl:.4f} ≤ piso ${self.daily_protect_floor:.4f} (check {self._daily_protect_below_count}/3)')
            else:
                # PnL voltou acima do piso — reseta contador de histerese
                self._daily_protect_below_count = 0
    ...
    def _check_exit(self, pair, price):
        for key, pos in list(self.positions.items()):
            ...
            _trail_sl_val = self.positions.get(key, {}).get('trail_sl')
            if side == 'BUY':
                active_sl = max(sl_orig, _trail_sl_val) if _trail_sl_val is not None else sl_orig
            else:
                active_sl = min(sl_orig, _trail_sl_val) if _trail_sl_val is not None else sl_orig
