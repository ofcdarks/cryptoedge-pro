...
class HFTEngine:
    def __init__(self, capital, client, notify_fn=None):
        ...
        log.info('  🚀 HFT Engine v3.1 ADAPTIVE + CONFIRMATION + CALIBRATION iniciado')
    ...
    def _try_confirm_and_enter(self, pair, open_, close, volume):
        ...
        # ── CONFIRMADO → entrar agora ─────────────────────────────────────
        log.info(f'  ✅ HFT {pair} {side} CONFIRMADO em ${close:,.4f} (drift={drift:.3f}%) | {pending["reason"]}')
        del self._pending[pair]

        entered = self._open_position(
            pair, side, close, f'[CONF] {pending["reason"]}',
            pending.get('strategies', []), pending.get('confidence', 0.5)
        )
        return entered
    ...
    def _open_position(self, pair, side, price, reason, strategies, confidence=0.5):
        ...
        tp_dist = max(atr_val * 2.0, sl_dist * min_rr)
    ...
    def _close_position(self, key, price, reason):
        ...
        log.info(f'  {icon} HFT FECHA {pair} | ${pnl:+.4f} | {int(duration)}s | {reason} | {total}T WR:{wr:.0f}% PnL:${self.daily_pnl:+.4f}')
        self.notify(
            f'{icon} HFT fechou {pair} | P&L: ${pnl:+.4f} ({int(duration)}s)\n'
            f'Hoje: {total} trades | WR:{wr:.0f}% | PnL: ${self.daily_pnl:+.4f}'
        )
    ...
    def send_daily_summary(self):
        ...
        self.notify(
            f'📊 Resumo Diário HFT v3.1\n'
            f'{icon} PnL: {"+"if self.daily_pnl>=0 else ""}${self.daily_pnl:.4f}\n'
            f'Trades: {total} ({self.daily_wins}W/{self.daily_losses}L) | WR: {wr:.1f}%\n'
            f'Por par:\n{pair_ln}\n'
            f'Top estratégias:\n  {st_ln}\n'
